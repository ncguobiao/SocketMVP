package com.gb.sockt.accountcenter.mvp.presenter.impl
...
import com.gb.sockt.accountcenter.mvp.presenter.LoginPresenter
import com.gb.sockt.accountcenter.mvp.service.UserService
import com.gb.sockt.accountcenter.mvp.view.LoginView
...
//                            L.i("jsonData="+jsonData)
                            val data = JSON.parseObject(jsonData, com.gb.sockt.accountcenter.data.domain.LoginBean::class.java)
                ...
                .execute(object : BaseSubscriber<BaseResp>(getView()!!) {
                    override fun onNext(t: BaseResp) {
                        if ("0000" == t.returnCode) {
                            val jsonData = t.retnrnJson.toString().trim()
//                            L.i("jsonData="+jsonData)
                            val data = JSON.parseObject(jsonData, com.gb.sockt.accountcenter.data.domain.WeiXinLoginSuccessBean::class.java)
//                            val jsonReader = JsonReader(StringReader(jsonData.toString()))//其中jsonContext为String类型的Json数据
//                            jsonReader.isLenient = true
//                            val data = Gson().fromJson<LoginBean>(jsonReader, LoginBean::class.java)
                            if (data != null) {
